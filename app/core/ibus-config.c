...
static void ibus_config_g_signal(GDBusProxy *proxy, const gchar *sender, const gchar *sigName, GVariant *params);

static void initable_iface_init(GInitableIface *initIface);

static void async_initable_iface_init(GAsyncInitableIface* asyncInitIface);

static gchar *_make_match_rule(const gchar *section, const gchar *name);

static guint _signal_subscribe(GDBusProxy *proxy);

static void _signal_unsubscribe(GDBusProxy* proxy, guint signalID);

static void _remove_all_match_rules(IBusConfig *config);

G_DEFINE_TYPE_WITH_CODE (IBusConfig,
                         ibus_config,
                         IBUS_TYPE_PROXY,
...
static void ibus_config_class_init(IBusConfigClass *class)
{
     ...
     */
    config_signals[VALUE_CHANGED] = g_signal_new (
            g_intern_static_string("value-changed"),
...
static void
ibus_config_g_signal(GDBusProxy *proxy, const gchar *sender, const gchar *signal_name, GVariant *parameters)
{
    if (g_strcmp0 (signal_name, "ValueChanged") == 0) {
        ...
        g_signal_emit (proxy, config_signals[VALUE_CHANGED], 0, section, name, value);
...
static void connection_signal_cb(GDBusConnection *conn, const gchar *sender, const gchar *objPath, const gchar *ifaceName, const gchar* sigName, GVariant *params, IBusConfig *config)
{
    g_return_if_fail (IBUS_IS_CONFIG (config));

    ibus_config_g_signal (G_DBUS_PROXY (config), sender, sigName, params);
...
static void _remove_all_match_rules(IBusConfig *config)
{
    for (int i = 0; i < config->priv->watch_rules->len; i++) {
...
gboolean ibus_config_unwatch(IBusConfig *config, const gchar *section, const gchar *name)
{
    ...
    if (retval && (section != NULL || name != NULL)) {
        ...
        for (i = 0; i < config->priv->watch_rules->len; i++) {
            gchar *_rule = g_array_index (config->priv->watch_rules, gchar *, i);
            if (g_strcmp0 (_rule, rule) == 0) {
                config->priv->watch_rules = g_array_remove_index_fast (config->priv->watch_rules, i);
                g_free (_rule);
                break;
            }
        }
...
IBusConfig *ibus_config_new(GDBusConnection *connection, GCancellable *cancellable, GError **error)
{
    ...
    GDBusProxyFlags flags = G_DBUS_PROXY_FLAGS_DO_NOT_AUTO_START | G_DBUS_PROXY_FLAGS_DO_NOT_LOAD_PROPERTIES |
                            G_DBUS_PROXY_FLAGS_DO_NOT_CONNECT_SIGNALS;

    initable = g_initable_new (IBUS_TYPE_CONFIG, cancellable, error, "g-connection", connection, "g-flags", flags,
                               "g-name", IBUS_SERVICE_CONFIG, "g-interface-name", IBUS_INTERFACE_CONFIG,
                               "g-object-path", IBUS_PATH_CONFIG, "g-default-timeout", ibus_get_timeout (), NULL);
    if (initable == NULL)
        return NULL;
    ...
    if (owner == NULL) {
        ...
        g_set_error (error, IBUS_ERROR, IBUS_ERROR_NO_CONFIG, "Configuration daemon is not running.");
...
void ibus_config_new_async(GDBusConnection *conn, GCancellable *cancel, GAsyncReadyCallback cb, gpointer udata)
{
...
GVariant *ibus_config_get_value(IBusConfig *config, const gchar *section, const gchar *name)
{
    ...
    result = g_dbus_proxy_call_sync ((GDBusProxy *) config,
                                     ...
                                     g_variant_new ("(ss)", section, name),     /* parameters */
...
void ibus_config_get_value_async(IBusConfig *config, const gchar *section, const gchar *name, gint timMs, GCancellable *cancel, GAsyncReadyCallback cb, gpointer udata)
{
    g_assert (IBUS_IS_CONFIG (config));
    g_assert (section != NULL);
    g_assert (name != NULL);
...
GVariant *ibus_config_get_value_async_finish(IBusConfig *config, GAsyncResult *result, GError **error)
{
    ...
    GVariant *value = NULL;
    GVariant *retval = g_dbus_proxy_call_finish ((GDBusProxy *) config, result, error);
    if (retval != NULL) {
...
GVariant *ibus_config_get_values(IBusConfig *config, const gchar *section)
{
    ...
    GError *error = NULL;
    GVariant *result;
    result = g_dbus_proxy_call_sync ((GDBusProxy *) config, "GetValues", g_variant_new ("(s)", section),
...
void ibus_config_get_values_async(IBusConfig *config, const gchar *section, gint timMs, GCancellable *cancel, GAsyncReadyCallback cb, gpointer udata)
{
    g_assert (IBUS_IS_CONFIG (config));
    g_assert (section != NULL);
...
gboolean ibus_config_set_value(IBusConfig *config, const gchar *section, const gchar *name, GVariant *value)
{
    ...
    GError *error = NULL;
    GVariant *result;
    result = g_dbus_proxy_call_sync ((GDBusProxy *) config, "SetValue",                /* method_name */
                                     g_variant_new ("(ssv)", section, name, value),  /* parameters */
                                     G_DBUS_CALL_FLAGS_NONE,    /* flags */
                                     -1,                        /* timeout */
                                     NULL,                      /* cancellable */
                                     &error);
...
void ibus_config_set_value_async(IBusConfig *config, const gchar *section, const gchar *name, GVariant *value, gint timMs, GCancellable *cancel, GAsyncReadyCallback cb, gpointer udata)
{
    g_assert (IBUS_IS_CONFIG (config));
    g_assert (section != NULL);
    g_assert (name != NULL);
    ...
    g_dbus_proxy_call ((GDBusProxy *) config, "SetValue",                /* method_name */
                       g_variant_new ("(ssv)", section, name, value),  /* parameters */
                       G_DBUS_CALL_FLAGS_NONE,    /* flags */
                       timMs, cancel, cb, udata);
}
...
gboolean ibus_config_unset(IBusConfig *config, const gchar *section, const gchar *name)
{
    ...
    GError *error = NULL;
    GVariant *result;
    result = g_dbus_proxy_call_sync ((GDBusProxy *) config, "UnsetValue",              /* method_name */
                                     g_variant_new ("(ss)", section, name),         /* parameters */
                                     G_DBUS_CALL_FLAGS_NONE,    /* flags */
                                     -1,                        /* timeout */
                                     NULL,                      /* cancellable */
...
static guint _signal_subscribe(GDBusProxy *proxy)
{
    ...
    return g_dbus_connection_signal_subscribe (connection, NULL, IBUS_INTERFACE_CONFIG, NULL, NULL, NULL,
                                               G_DBUS_SIGNAL_FLAGS_NO_MATCH_RULE,
                                               (GDBusSignalCallback) connection_signal_cb, g_object_ref (proxy),
...
static gboolean initable_init(GInitable *initable, GCancellable *cancellable, GError **error)
{
    ...
    config->priv->watch_config_signal_id = _signal_subscribe (G_DBUS_PROXY (initable));

    gboolean retval = ibus_config_watch (config, NULL, NULL);
    if (!retval)
        g_set_error (error, IBUS_ERROR, IBUS_ERROR_FAILED, "Cannot watch configuration change.");
    return retval;
}

static void initable_iface_init(GInitableIface *initableIface)
{
    initable_iface_parent = g_type_interface_peek_parent (initableIface);
    initableIface->init = initable_init;
}

static GAsyncInitableIface *async_initable_iface_parent = NULL;

static void async_initable_init_async(GAsyncInitable *initable, gint ioPriority, GCancellable *cancel, GAsyncReadyCallback cb, gpointer udata)
{
    async_initable_iface_parent->init_async (initable, ioPriority, cancel, cb, udata);
}
...
static void async_initable_iface_init(GAsyncInitableIface *asyncInitableIface)
{
    async_initable_iface_parent = g_type_interface_peek_parent (asyncInitableIface);
    asyncInitableIface->init_async = async_initable_init_async;
    asyncInitableIface->init_finish = async_initable_init_finish;
}
