...
#define IBUS_SERIALIZABLE_GET_PRIVATE(o)    ((IBusSerializablePrivate *)ibus_serializable_get_instance_private (o))
...
static gboolean ibus_serializable_real_serialize (IBusSerializable* object, GVariantBuilder* builder);
static gint ibus_serializable_real_deserialize (IBusSerializable* object, GVariant* variant);
static gboolean ibus_serializable_real_copy (IBusSerializable* dest, const IBusSerializable* src);

static IBusObjectClass *parent_class = NULL;
static gint ibus_serializable_private_offset;

G_GNUC_UNUSED
static inline gpointer ibus_serializable_get_instance_private (IBusSerializable *self)
{
...
GType ibus_serializable_get_type (void)
{
    ...
    if (type == 0) {
        type = g_type_register_static (IBUS_TYPE_OBJECT, "IBusSerializable", &type_info, 0);
        ibus_serializable_private_offset = g_type_add_instance_private (type, sizeof (IBusSerializablePrivate));
    }
...
static void ibus_serializable_class_init (IBusSerializableClass *class)
{
    ...
    if (ibus_serializable_private_offset) {
        g_type_class_adjust_private_offset (class, &ibus_serializable_private_offset);
    }
...
static void ibus_serializable_init (IBusSerializable *serializable)
{
...
static void ibus_serializable_destroy (IBusSerializable *serializable)
{
    g_datalist_clear (&serializable->priv->attachments);
    parent_class->destroy (IBUS_OBJECT (serializable));
}

static void serialize_cb (GQuark key, GVariant* value, GVariantBuilder* array)
{
    g_variant_builder_add (array, "{sv}", g_quark_to_string (key), g_variant_new_variant (value));
}

static gboolean ibus_serializable_real_serialize (IBusSerializable *serializable, GVariantBuilder* builder)
{
    GVariantBuilder array;
    g_variant_builder_init (&array, G_VARIANT_TYPE ("a{sv}"));

    g_datalist_foreach (&serializable->priv->attachments, (GDataForeachFunc) serialize_cb, &array);
...
static gint ibus_serializable_real_deserialize (IBusSerializable *object, GVariant* variant)
{
    ...
    while (g_variant_iter_loop (iter, "{&sv}", &key, &value)) {
        ...
        ibus_serializable_set_attachment (object, key, attachment);
...
static void copy_cb (GQuark key, GVariant* value, GData** datalist)
{
...
GVariant* ibus_serializable_get_qattachment (IBusSerializable *serializable, GQuark key)
{
    ...
    return (GVariant*) g_datalist_id_get_data (&serializable->priv->attachments, key);
}
...
GVariant* ibus_serializable_serialize_object (IBusSerializable* object)
{
...
IBusSerializable* ibus_serializable_deserialize_object (GVariant* variant)
{
