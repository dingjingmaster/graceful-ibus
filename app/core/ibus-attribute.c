...
static gboolean ibus_attribute_serialize (IBusAttribute* attr, GVariantBuilder* builder)
{
    gboolean retval;
...
static gint ibus_attribute_deserialize (IBusAttribute* attr, GVariant* variant)
{
    gint retval;
...
static gboolean ibus_attribute_copy (IBusAttribute* dest, const IBusAttribute* src)
{
    gboolean retval;

    retval = IBUS_SERIALIZABLE_CLASS (ibus_attribute_parent_class)->copy ((IBusSerializable *) dest, (IBusSerializable *) src);
    g_return_val_if_fail (retval, FALSE);
...
{
    g_return_val_if_fail (type == IBUS_ATTR_TYPE_UNDERLINE  || type == IBUS_ATTR_TYPE_FOREGROUND || type == IBUS_ATTR_TYPE_BACKGROUND, NULL);
